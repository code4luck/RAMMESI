...
class Attention1dPooling(nn.Module):
    ...
    def forward(self, x, input_mask=None):
        ...
        if input_mask is not None: # [B, L, 1]
            attn = attn.masked_fill_( # 0 is padding, 1 is not padding
...
class Attention1dPoolingProjection(nn.Module):
    def __init__(self, hidden_size, num_labels=1, dropout=0.25) -> None:
...
class Attention1dPoolingHead(nn.Module):
    ...
    ):  # [batch x sequence x embedding ] --> [batch x embedding]
        super(Attention1dPoolingHead, self).__init__()
        self.attention1d = Attention1dPooling(hidden_size)
        self.proj = Attention1dPoolingProjection(hidden_size=hidden_size)
...
class LightAttentionPoolingHead(nn.Module):
    def __init__(self, hidden_size=1280 , dropout=0.25, kernel_size=9, conv_dropout: float = 0.25):
    ...
    def forward(self, x: torch.Tensor, mask, **kwargs) -> torch.Tensor:
        ...
        Returns:
            classification: [batch_size,hidden_size] tensor with logits
        """
        #  0 is padding, 1 is not padding
        # check if mask is bool
        ...
        o = self.linear(o)  # [batchsize, hidden_size]
        
        return self.output(o)  # [batchsize, hidden_size]
...
class MeanPooling(nn.Module):
    ...
    def forward(self, features, input_mask=None):
        if input_mask is not None:
            # Applying input_mask to zero out masked values 0 is padding, 1 is not padding
...
class MeanPoolingHead(nn.Module):
    ...
    def __init__(self, hidden_size, dropout=0.25):
        ...
        self.mean_pooling_projection = MeanPoolingProjection(hidden_size,dropout)
